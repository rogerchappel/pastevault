...
- Mutating commands serialize through a short-lived adjacent lock file that records the writer PID, host, creation time, and a unique ownership token, so concurrent successful commands preserve every update. A command fails with an actionable lock error after 10 seconds if another writer does not release the lock. Locks older than 30 seconds are recovered only when their complete ownership record names a same-host process that is no longer alive. Locks with unknown ownership, locks from another host, and locks owned by a live process are never stolen; an unknown lock must be inspected and removed manually after confirming no writer is active. Handled success and failure remove lock and temporary files. PID liveness is necessarily host-local, so copy a vault between machines only when no writer is active.
